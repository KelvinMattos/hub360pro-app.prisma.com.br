...
class AmazonAdapter implements MarketplaceAdapter
{
    ...
    {
        Log::info("Amazon: Updating price for {$externalId} to {$price}");
        return true;
    }

    public function updateStock(Integration $credential, string $externalId, int $stock)
    {
        Log::info("Amazon: Updating stock for {$externalId} to {$stock}");
        return true;
    }
    ...
    {
        Log::info("Amazon: Sending message to order {$orderId}");
        return true;
    }
