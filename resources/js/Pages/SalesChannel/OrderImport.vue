        ...
        <div class="p-6 lg:p-8 max-w-5xl mx-auto">
            ...
            <div class="mb-8">
                <div class="flex items-center gap-2 text-xs font-bold uppercase tracking-widest text-slate-400 mb-2">
                    <i class="fa-solid fa-file-import"></i> Importações de Vendas por Marketplace
                </div>
            ...
            <div class="flex flex-wrap gap-2 mb-8">
                <Link v-for="t in allTypes" :key="t.key" :href="route('order-channel.show', { type: t.key })"
            ...
            <div class="bg-teal-50 border border-teal-200 text-teal-800 text-sm px-4 py-3 rounded-xl mb-6">
                ...
                Cria/atualiza <b>pedidos</b>{{ config.kind === 'items' ? ' e itens' : '' }}, cruzando pelo
            ...
            <div v-if="!accounts.length" class="bg-amber-50 border border-amber-200 text-amber-800 text-sm px-4 py-3 rounded-xl mb-6 flex items-center justify-between gap-4">
                <span><i class="fa-solid fa-triangle-exclamation mr-2"></i>Nenhuma conta cadastrada pra este canal ainda. Cadastre pelo menos uma antes de importar.</span>
                <Link :href="route('sales.channel-accounts.index')" class="btn-secondary shrink-0">Cadastrar conta</Link>
            </div>
            ...
            <div class="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm">
                ...
                <label :class="['dz block', form.file ? 'dz-filled' : '', !accounts.length ? 'opacity-50 pointer-events-none' : '']" @dragover.prevent @drop.prevent="onDrop">
                    <div class="flex items-center gap-4">
                        ...
                        <div>
                            <div class="font-semibold text-slate-700">{{ form.file ? form.file.name : `Clique ou arraste o export do canal aqui` }}</div>
                            <div class="text-xs text-slate-400 mt-0.5">{{ form.file ? fileSize(form.file.size) : 'Arquivo original do canal · até 120 MB' }}</div>
                        </div>
                    </div>
            ...
            <div v-if="showResult && result" class="fixed inset-0 z-[9999] bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4" @click.self="showResult = false">
                <div class="bg-white rounded-2xl shadow-2xl w-full max-w-lg p-8">
                    ...
                    <div v-if="result.ok" class="grid grid-cols-3 gap-3 mt-6">
                        <div class="stat"><div class="stat-v">{{ n(result.rows) }}</div><div class="stat-l">pedidos</div></div>
                        <div class="stat"><div class="stat-v text-emerald-600">{{ n(result.created) }}</div><div class="stat-l">criados</div></div>
                        <div class="stat"><div class="stat-v text-slate-400">{{ n(result.updated) }}</div><div class="stat-l">atualizados</div></div>
                    </div>
...
function submit() {
    ...
    form.post(route('order-channel.import', { type: props.type, progress_token: token }), {
        preserveScroll: true,
        forceFormData: true,
        onProgress: (e) => {
...
function startPoll(token) {
    ...
    pollTimer = setInterval(async () => {
        try {
            const url = route('order-channel.progress', token) + `?t=${Date.now()}`;
