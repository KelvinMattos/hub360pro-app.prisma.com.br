    ...
    <AppLayout>
        <div class="p-6 lg:p-8 max-w-5xl mx-auto">
            ...
            <div class="flex flex-wrap gap-2 mb-8">
                <Link v-for="t in allTypes" :key="t.key" :href="route('netshoes.show', { type: t.key })"
                    :class="['px-4 py-2 rounded-xl text-sm font-semibold border transition flex items-center gap-2',
                        t.key === type ? 'bg-orange-500 text-white border-orange-500 shadow-sm' : 'bg-white text-slate-600 border-slate-200 hover:bg-slate-50']">
            ...
            <div class="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm">
                <h2 class="text-xs font-black uppercase tracking-[0.15em] text-slate-400 mb-4">Enviar arquivo (.xlsx)</h2>
                ...
                <label :class="['dz block', form.file ? 'dz-filled' : '']" @dragover.prevent @drop.prevent="onDrop">
                    <div class="flex items-center gap-4">
                        ...
                        <div>
                            <div class="font-semibold text-slate-700">{{ form.file ? form.file.name : 'Clique ou arraste o arquivo .xlsx aqui' }}</div>
                            <div class="text-xs text-slate-400 mt-0.5">{{ form.file ? fileSize(form.file.size) : 'Export .xlsx do painel Netshoes · até 120 MB' }}</div>
                        </div>
                    </div>
            ...
            <div class="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm mt-6">
                ...
                <p class="text-xs text-slate-400 mt-4">
                    ...
                    Chave: <code class="k">{{ config.key_label }}</code> · Grava: <code class="k">{{ config.value_label }}</code>
                </p>
            </div>
        </div>
            ...
            <div v-if="phase !== 'idle'" class="fixed inset-0 z-[9999] bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4">
                <div class="bg-white rounded-2xl shadow-2xl w-full max-w-md p-8 text-center">
                    <div class="w-16 h-16 mx-auto mb-5 relative">
                        ...
                        <i class="fa-solid fa-database absolute inset-0 flex items-center justify-center text-orange-500 text-lg"></i>
                    </div>
                    ...
                    <template v-if="phase === 'uploading'">
                        ...
                        <div class="mt-4 h-2 bg-slate-100 rounded-full overflow-hidden">
                            <div class="h-full bg-orange-500 rounded-full transition-all" :style="{ width: uploadPct + '%' }"></div>
                        </div>
                    </template>
                    ...
                    <template v-else>
                        ...
                        <p class="text-sm text-slate-500 mt-1">
                            <span class="font-mono font-bold text-orange-600 text-lg">{{ n(live.done) }}</span>
...
function submit() {
    ...
    form.post(route('netshoes.import', { type: props.type, progress_token: token }), {
        preserveScroll: true,
        forceFormData: true,
        onProgress: (e) => {
...
function startPoll(token) {
    ...
    pollTimer = setInterval(async () => {
        try {
            const url = `/imports/netshoes/progress/${encodeURIComponent(token)}?t=${Date.now()}`;
...
<style scoped>
.btn-primary { @apply bg-orange-500 hover:bg-orange-600 text-white font-semibold rounded-lg px-5 py-2.5 transition shadow-sm; }
.dz { @apply border-[1.5px] border-dashed border-slate-200 rounded-xl p-6 cursor-pointer transition hover:border-orange-300; }
...
.k { @apply font-mono bg-slate-100 text-orange-700 px-1.5 py-0.5 rounded text-[11px]; }
